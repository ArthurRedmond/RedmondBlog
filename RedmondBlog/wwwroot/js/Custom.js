...
function AddTag() {
    ...
    if (searchResult != null) {
        ...
        swalWithDarkButton.fire({
            html: `<span class='fw-bold'>${searchResult.toUpperCase()}</span>`
        });
    }
    else {
...
function DeleteTag() {
    ...
    if (tagList.selectedIndex == -1) {
        swalWithDarkButton.fire({
            html: "<span class='fw-bold'>CHOOSE A TAG BEFORE DELETING</span>"
        });
        return true;
    }
...
//The search function will detect either an empty or duplicate Tag on the same post
function search(str) {
    if (str == "") {
